...
int main(void){
    ...
    fprintf(stderr,"STAGE:img_convert\n");
    ...
    fprintf(stderr,"STAGE:img_get_pixel32\n");
    int before = img_get_pixel32(&img,1,0);
    fprintf(stderr,"STAGE:img_fix_next_pixel\n");
    img_fix_next_pixel(&img,0,0,0x0000FF00u);
    fprintf(stderr,"STAGE:img_get_pixel32\n");
    int after = img_get_pixel32(&img,1,0);
    CHECK((after & 0x00FF0000u) > (before & 0x00FF0000u), "fix_next_pixel blends neighbour toward green");

    sakum_img inv = { buf, 4,4, 0, 0 };
    fprintf(stderr,"STAGE:img_fill\n");
    img_fill(&inv, 0xFFFFFFFFu);
    fprintf(stderr,"STAGE:img_editor_invert\n");
    ...
    fprintf(stderr,"STAGE:img_editor_grayscale\n");
    ...
    fprintf(stderr,"STAGE:img_reader_load_raw\n");
