...
| `sakum_tracker.s` | **ब्रम्ह LIVE HISTORY VIEWER** — the live self-update tracker, written in raw x86-64 (no Python, no host language). Reads `query_logs/fetch_live.jsonl` (the live history ledger) and prints the pipeline `स्रोत → भाषा → गंतव्य` plus the pulse clock. Replaces the dead `serve.py` + `sakum_status.sh`. | `gcc -arch x86_64 assembly/sakum_tracker.s -o /tmp/tracker && /tmp/tracker` (once) · `/tmp/tracker --live` (tail, refresh 3s) · `/tmp/tracker <path>` (custom feed) |
