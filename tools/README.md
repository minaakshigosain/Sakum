...
| `serve.s` | **Native** Sakum trigger server — raw x86-64 AT&T assembly (no libc net wrappers beyond the syscalls, no host-language interpreter). `POST /update` runs a bot cycle; `GET /status` dumps `memory.md`; `GET /nerve` prints the nerve-bus firing counts (`webhook.update` / `ws.trigger` / `timer.pulse`); `GET /` and `GET /index.html` serve the `site/index.html` web dashboard (`Content-Type: text/html`); any other path → `404`. Build/run: `gcc -arch x86_64 tools/serve.s -o /tmp/serve && /tmp/serve --http 8080 --pulse 600`. WebSocket is intentionally dropped (needs SHA1/base64 masking, out of scope); timer pulse via a forked child. **Replaces `serve.py` (retired — Python is a host-language runtime, forbidden by SAKUM_LANG.md §2).** |
...
| `fix_perms.sh` | Re-asserts user ownership + `666` on repo output files (`ai_ledger.txt`, `site/index.html`) and `755` on the `/tmp` native binaries. Guards against a prior `sudo` server run leaving the ledger root-owned (which makes the AI's self-update silently write nothing). Called automatically by `serve.sh`. |
| `check.sh` | Doctrine-compliant functional smoke test (bash + native binaries only). Builds `sakum_pipeline.s`/`sakum_scan.s`/`sakum_ai.s`, asserts `result: 186`, spins up the native trigger server and confirms the scanner detects its port, verifies the AI core loads 85 chunks / 64 neurons / 0 leaks and appends to its ledger, then runs a bot cycle and confirms `site/index.html` is refreshed. Run: `bash tools/check.sh`. |
...
bash tools/serve.sh 8080 600                      # native trigger server (8080/600)
bash tools/check.sh                               # functional smoke test
```
